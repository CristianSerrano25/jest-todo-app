...
describe('Todo API', () => {
  // Limpiar los todos antes de cada test
  beforeEach(async () => {
    // Obtener todos los todos y eliminarlos uno por uno
  ...
  it('GET /todos/stats devuelve los totales', async () => {
    ...
    const a = await request(app).post('/todos').send({ title: 'AA' });
    const b = await request(app).post('/todos').send({ title: 'BB' });
    const c = await request(app).post('/todos').send({ title: 'CC' });
